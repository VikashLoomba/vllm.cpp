...
| ROCm (`BACKEND-GATE-ROCM-VLLM` / `-SGLANG`) | **NOT APPLICABLE: no new number.** W0 ctest-green on 4 gfx archs (#41); #140 ran M0/M1 on 2× R9700; #158 Task 1 expert-GeGLU fold is CPU byte-exact, Task 2 pending. No AMD HW here | The approach-(b) fix (PENDING community) unblocks the first APU model run (M2); the gate becomes a same-box vLLM-ROCm oracle once a model runs ([#41](https://github.com/mudler/vllm.cpp/issues/41)); floor: vLLM |
