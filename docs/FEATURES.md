...
| MiniMax-H3 DiT (`MiniMaxH3DiTModel`, vllm-omni lane) | MiniMax-H3 (33.1B video+audio) | portable path 66/66 (DiT geometry ladder + CUDA-vs-host at seq 1920); t2va renders a COHERENT prompt-matched scene on GB10; encoder vision tower now loads real `visual.*` weights (was synthetic-only) | FP4/Marlin routing landed; conditioned-render + speed pending |
...
| Video+audio GENERATION (MiniMax-H3 DiT, vLLM-Omni lane) | ◐ t2va renders a COHERENT prompt-matched scene on GB10; task/partition guard mirrors `_resolve_task`; Marlin W4A16 byte-exact; conditioning: vision tower now loads real weights (§8.8), render pending | ✅ (vllm-omni, BF16-only, no quantized H3 arm) | ☐ | ☐ |
