...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **t2va+fl2va COHERENT; ref2va grid = ckpt quant not loader; bf16 13-shard DiT INDEXES** | act diff: weights/islands/RoPE quant-noise-close to GGUF; residual = community-NVFP4 fidelity §8.12 |
...
Parakeet/FastConformer ASR (2026-08-07): *correctness-complete, CPU only*. Kernels, encoder, CTC greedy, log-mel; gated vs a HF `ParakeetForCTC` oracle, ids exact; real transcripts on ctc-0.6b/1.1b, rnnt-0.6b, tdt-0.6b-v3. No CUDA, no aarch64, no speed number.

## Not supported yet

LoRA (W1 CPU runtime brick landed; not yet usable end-to-end), multi-GPU,
Vulkan (16 native kernels, 71 on CPU tier; opt-125m e2e
token-exact, coopmat 11-33x Thor;
