...
| **W4** | Video VAE. **DECODER DONE** — the full ViT3D decoder gated at 8.9e-8 (block 6.0e-8), real hyperparameters 36 layers / 32 heads x 64 / rope_theta 100 / rope_dim_ratio 0.75 from the checkpoint's `vit_decoder_kwargs`. **TILING also DONE** (plan + seam blend, exact). **3D-CNN ENCODER primitives also DONE** (causal Conv3d with reflect spatial padding, GroupNorm3D, ResnetBlock3D). REMAINS: the encoder's Downsample3D + EncoderFCN3D assembly — conditioning-only, not needed for generation output | — |
