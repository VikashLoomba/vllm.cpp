...
| 35B fresh grid | **BOUND** @`1ea26427`: tput 0.93-1.03x, c16 0.93x. INTAKE lever **RESOLVED NEGATIVE**; Option A (H2D out of capture) also **RESOLVED NEGATIVE** (tput WASH) | Real lever left: prefill glue (task #61) |
| Async decode: Option A (`VT_ASYNC_EXECUTOR`) LANDS gated **OFF** (`row/SERVE-ASYNC-OPTION-A`) | Input H2D staged OUT of capture (persistent dev buf + pinned + staged event, `states.py:64`); RED **now reproduces** (pinned poison); GREEN 5/5 c32, SACRED 3/3, memcheck 0-err. Binding A/B **WASH** -> c16 gap is NOT the drain/baked-H2D. Detail in state | prefill glue |
