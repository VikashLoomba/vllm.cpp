...
| `vllm-project/vllm-gguf-plugin` | `vllm-gguf-plugin` | GGUF, which the pinned vLLM carries no in-tree support for: it was deprecated in `vllm#39583` and moved to this first-party repository, so a GGUF path it serves is a path the PRIMARY oracle serves and llama.cpp is NOT admissible for it |
