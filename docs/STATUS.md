...
| Qwen3.6-35B-A3B (NVFP4, GDN MoE) | Correctness-complete; binding grid @`a0fa12c7` FLAT 0.935x-0.979x; gate_up +1.31% and down bf16 +2.05% LANDED (bit-exact); SiLU 9.2x/launch spec'd; PSS 3.8x | Token-exact SYNC+ASYNC; `VT_ASYNC_DEVICE_MIRROR` ON fixes async batch-1 token-0 degeneration |
