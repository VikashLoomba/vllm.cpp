...
| **W7** | Serving. **Output path + API logic DONE**: PPM frames + WAV + the MP4 mux argv (validated end-to-end against real ffmpeg), plus the `/v1/videos` request contract and the job store (lifecycle, status JSON, thread-safe). REMAINS: the mechanical glue — registering the two routes on `ApiServer` and injecting a `VideoRunner`, whose ffmpeg invocation lives in `examples/` per the developer's ratified decision | W6 |
