...
| Startup latency (cold launch to first `/health`) | **PENDING: no number measured, claimed or owed yet.** Harness landed 2026-08-06 (`--startup-only`, 0.2 s cadence, cold-gated). [Detail](../.agents/specs/startup-latency-axis.md) | Capture on an idle box |
...
| Parakeet/FastConformer ASR (P1-P6) | **No number measured, claimed or owed.** Correctness-gated only, CPU f32. Transcripts verified on `parakeet-ctc-0.6b`, `-ctc-1.1b`, `-rnnt-0.6b`, `-tdt-0.6b-v3`. | Floor is parakeet.cpp when a number is owed |
