...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **t2va+fl2va COHERENT; vision→cond scatter wired+gated** | fl2va matched cat via keyframe+encoder-vision; merged→prompt_embeds+DeepStack gated; ref2va grids (residual=ref2va ref rows). §8.6-8.9. |
