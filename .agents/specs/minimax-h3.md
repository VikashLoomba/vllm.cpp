...
| **W7** | Serving. **Output path DONE**: PPM frame serialization + WAV + the MP4 mux argv, validated end-to-end against real ffmpeg. The library never spawns a process — `src/vllm/` has no subprocess precedent — so the INVOCATION belongs to the example/server layer, which is what remains along with `/v1/videos` + `/v1/videos/sync` and the job store | W6 |
