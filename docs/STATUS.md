...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **Portable lane COMPLETE; e2e video RAN (Thor); FP4 routing CPU-landed, GB10 leg IN FLIGHT** | Portable path COMPLETE (DiT 1.6e-7, VAEs, encoder, t2va, GGUF+NVFP4 loaders, /v1/videos); e2e video (Thor); W-FP4a fp4-resident Marlin-W4A16 routing CPU-gated. GB10 leg (`row/H3-FP4-GPU-E2E`): CUDA Marlin-W4A16 speed case (asserts `marlin_gemms==11`) + `--fp4-resident` driver flag landed; dgx delta + s/step measurement in flight. Detail: spec §8. |
