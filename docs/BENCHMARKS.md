...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Both arms measured, same weights.** llama.cpp Vulkan `NV_coopmat2`: pp128 **11,514 t/s**, tg32 **160.9 t/s**. vllm.cpp Vulkan: prefill **23.0**, decode **5.85 t/s** | ~500x prefill / ~27x decode behind. Levers MEASURED; 2 wrong (CPU tier fires ONCE/run; `VK-A2` caps **1.15x**). GEMV **1.8x** GEMM (7/8), argmax **18.9x**, opt-125m 6/6 exact. Subgroup reduction VOID (4/8 wash) |
