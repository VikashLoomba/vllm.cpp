...
the activation and output types, and grouped delegation. The operator owns the
HIP gate. This stage makes no performance claim for gfx1100 or gfx1151.
...
- T4a host gate: the CPU build runs the MMVQ route-policy case in
  `test_rocm_quant_dot`. The operator's HIP build must run that binary with
  `--no-skip` before T4a can claim device correctness.
