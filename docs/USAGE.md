...
**A `glm5next` file LOADS. The model's forward is WIRED, and the ENGINE still
refuses one level ABOVE it, so no token can be generated yet.** W1 ([#2067](https://github.com/mudler/vllm.cpp/issues/2067)) gave
...
**THE ENGINE PATH DOES NOT REACH THAT HOOK, and this was MEASURED rather than
assumed.** Driven at the staged 101.2535 GiB artifact on `dgx:gpu0` on
2026-08-30 ([#2343](https://github.com/mudler/vllm.cpp/issues/2343)), all four
shards load and the engine sizes its caches -- `max_model_len` auto-fits from
1048576 to 8192 against 256 blocks of 32 tokens, and `max_num_seqs` drops from
32 to 1 because one 4,390,912-byte GDN state fills a unified page -- and then the
FIRST step throws:
...
model's hook, and GLM-5.3-Flash publishes three groups (W5,
[#2223](https://github.com/mudler/vllm.cpp/issues/2223)). `vllm-cli` exits with
`completion failed (status 3)`. The consuming forward is owed by that row and not
by this one.
