...
| Kimi-Linear-48B-A3B (KDA + NoPE-MLA + MoE hybrid) | KDA `vt::KdaGatedDeltaRule` GB10 122/128, 4.24 tok/s (recompute), NOT STRICT, OFF. **Paged-incremental decode IMPLEMENTED (§19), CPU byte-exact state-carry gated** | `test_kimi_linear_forward` **15/15·875** (decode == full-recompute byte-exact + greedy-identical); GB10 Gate A/B + speed vs vLLM ~21 PENDING |
