...
provider from issue #2781 is reachable; its HIP compile and runtime gate remain
pending with the operator. The unmerged campaign's later opt-in arms, default
changes, and product changes are not reachable from this tree. The measured
position and next hypothesis that follow are historical evidence from the
source commit. They are not a current-main benchmark.
