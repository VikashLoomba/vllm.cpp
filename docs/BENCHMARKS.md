...
| **Record/checker repair 2026-08-07–08** | Python 3.12/3.13-stable AST pins; Gemma-4 drift and landed/PARTIAL embeddings recorded; first-parent/PARTIAL mutation pins; exact `MANIFESTO.md` public-doc class. No runtime/performance change |
...
| Embeddings on the ONE surface (ROW 6, `LlamaModel` + `vllm_embed` + `/v1/embeddings`) | **PARTIAL (`57ed063e`); NO speed claim.** CPU fixture: engine path == direct registry path; f64 LAST+normalize reference. No number measured or owed | A REAL embedding checkpoint (e5-mistral class) + a same-box `vllm.LLM(task="embed")` oracle; only then does an embed-throughput bar exist |
