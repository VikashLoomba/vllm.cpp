...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **ONE SURFACE ROW 2: ABI v12 `vllm_video_*`; `/v1/videos` + thin-client examples on ONE seam, frames+WAV byte-exact; ratchet 9.** t2va+fl2va COHERENT; bf16 shards STREAM | ref2va = NVFP4 ckpt fidelity §8.12; encoder A/B §8.15; GB10 re-verify via the v12 ABI = named residual; embedder-unreachable caps now 2 |
