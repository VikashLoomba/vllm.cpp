...
// Warp-per-query with SHARED-MEMORY K/V TILING.
//
// The plain warp kernel re-reads all of K and V from GLOBAL memory for every
// query, so traffic is O(seq^2 * d) per head. That is invisible at short
// sequences and dominant at long ones: MiniMax-H3 at 864x480/124f measured
// 557 s/step at seq_len 15424, where 4.7x the sequence of a 512x512 run cost 29x
// the time -- i.e. ~85% of the step was attention, and memory-bound rather than
// FLOP-bound.
//
// Here the warps of a block cooperatively stage a tile of K and V into shared
// memory and each warp evaluates its own query against that tile, cutting global
// K/V traffic by the number of warps per block.
//
// The online-softmax recurrence and the KEY ORDER are unchanged, so results are
// identical to the untiled warp kernel, not merely close.
//
// GUARDED to the non-causal, single-document case (`num_reqs == 1`, no sliding
// window): only then do all warps in a block share one key range, which is what
// makes a shared tile correct. Causal / multi-request work keeps the per-warp
...
__global__ void DFlashBlockAttentionWarpTiledKernel(Tout* out, const Tin* query, const Tin* key,
  ...
  float* vs = smem + static_cast<int64_t>(kTile) * d;  // [kTile][d]

  const int lane = threadIdx.x & 31;
  const int warp = static_cast<int>(threadIdx.x >> 5);
  const int64_t i = static_cast<int64_t>(blockIdx.x) * kWarps + warp;  // query row
...
#pragma unroll
  for (int c = 0; c < kPerLane; ++c) {
    qreg[c] = active ? Load(query, qoff + static_cast<int64_t>(c) * 32 + lane) : 0.0f;
    acc[c] = 0.0f;
  }
  ...
  for (int64_t base = 0; base < rows; base += kTile) {
    ...
    for (int64_t e = threadIdx.x; e < n * d; e += threads) {
      ...
      const int64_t off = ((base + r) * hk + g) * d + c;
      ks[e] = Load(key, off);
      vs[e] = Load(value, off);
    }
    __syncthreads();
    ...
    if (active) {
      for (int64_t r = 0; r < n; ++r) {
        const float* krow = ks + r * d;
        float part = 0.0f;
#pragma unroll
        for (int c = 0; c < kPerLane; ++c) {
          part += qreg[c] * krow[static_cast<int64_t>(c) * 32 + lane];
        }
#pragma unroll
        ...
        const float* vrow = vs + r * d;
#pragma unroll
        for (int c = 0; c < kPerLane; ++c) {
          acc[c] = acc[c] * corr + pw * vrow[static_cast<int64_t>(c) * 32 + lane];
        }
        l = l * corr + pw;
        m = m_new;
      }
    }
    __syncthreads();  // tile is about to be overwritten
  }
...
void LaunchDFlashBlockAttention(cudaStream_t s, Tensor& out, const Tensor& query,
  ...
  // Long single-document sequences are MEMORY-bound on re-reading K/V per query,
  // so stage them in shared memory and share the tile across the block's warps.
  ...
  if (d % 32 == 0 && d / 32 <= 4 && !args.causal && args.sliding_window == 0 &&
    ...
    const dim3 tgrid(static_cast<unsigned>((t + kWarps - 1) / kWarps), static_cast<unsigned>(hq));
#define VT_DFLASH_TILED(PER_LANE)                                                                \
  do {                                                                                           \
    ...
    switch (d / 32) {
      case 1: VT_DFLASH_TILED(1); break;
      case 2: VT_DFLASH_TILED(2); break;
      case 3: VT_DFLASH_TILED(3); break;
      default: VT_DFLASH_TILED(4); break;
    }
#undef VT_DFLASH_TILED
    Check(cudaGetLastError(), "dflash-block-attn tiled launch");
