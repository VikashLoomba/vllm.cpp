...
| 35B fresh grid | **BOUND** @`1ea26427`: tput 0.93-1.03x, c16 0.93x. INTAKE lever **RESOLVED NEGATIVE**: attribution boundary (drain A/B collapses intake -91% but shifts to queued; arrival→sched GPU-bound, NEUTRAL). Probe kept, lever reverted | Real lever: prefill glue (task #61); async executor resolved (row above) |
| Async-serving decode: mirror fix MERGED (#31). Follow-up: decode-graph double-buffer (`row/SERVE-ASYNC-EXECUTOR`, `VT_ASYNC_EXECUTOR`) | Landed gated **OFF**. Ring+reuse-event replaces the depth-2 drain; proven correct (GREEN 5/5 @c32, SACRED 3/3; capture-malloc fixed). RED unreproducible on GB10 (H2D window tiny) → OFF. See state | Option A follow-up |
