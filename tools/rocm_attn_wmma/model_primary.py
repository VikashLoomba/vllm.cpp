...
def main():
    ...
    if args.mode == "generate":
        ...
        prompts = [
            ...
            "Read these observations: the garden contains roses, lavender, and mint. " * 36
            + "\nSummarize the observations in one short sentence.",
            "The river flows through the valley and provides water for the village. " * 92
        ...
        args.manifest.write_text(json.dumps(dict(output_len=32, cases=cases), indent=2) + "\n")
    ...
    if args.mode == "dtype":
        def inspect_head(model):
            ...
            hidden = torch.zeros((1, weight.shape[1]), device=weight.device, dtype=weight.dtype)
            logits = model.compute_logits(hidden)
            return dict(weight_dtype=str(weight.dtype), hidden_dtype=str(hidden.dtype),
                        projection_dtype=str(logits.dtype),
                        configured_head_dtype=str(model.logits_processor.head_dtype))
        args.output.write_text(json.dumps(llm.apply_model(inspect_head), indent=2) + "\n")
        return
    ...
    if args.mode == "bench":
        ...
        if "warmup" in manifest:
            warm = llm.generate([dict(prompt_token_ids=manifest["warmup"]["prompt_token_ids"])], sampling)[0]
        ...
        metrics, outputs = helper.derive_metrics(records, ids, duration,
            output_len=manifest["output_len"], max_concurrency=1, async_scheduling="default")
        report.update(metrics=metrics, records=records, prompt_token_ids=ids, output_token_ids=outputs)
        args.output.write_text(json.dumps(report, indent=2) + "\n")
        print("BENCH", json.dumps(metrics), flush=True)
        return
    for entry in manifest["cases"]:
        start = time.perf_counter()
        result = llm.generate([dict(prompt_token_ids=entry["prompt_token_ids"])], sampling)[0]
        seconds = time.perf_counter() - start
        out = result.outputs[0]
        report["cases"].append(dict(name=entry["name"], prompt_token_ids=result.prompt_token_ids,
            output_token_ids=list(out.token_ids), text=out.text, seconds=seconds,
            logprobs=[{str(k): v.logprob for k, v in row.items()} for row in out.logprobs]))
