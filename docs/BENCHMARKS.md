...
| MiniMax-H3 FP4 speed (W-FP4a) | Pending (measurement in flight, `row/H3-FP4-GPU-E2E`). fp4-resident Marlin-W4A16 routing CPU-landed (63/63); the GB10 CUDA speed case (asserts `marlin_gemms==11`, records fp4-vs-bf16 delta + per-forward/per-GEMM times) and `--fp4-resident` driver flag landed; dgx build+run + real e2e in flight; vLLM-Omni has no quantized H3 (BF16-only) | Build+run CUDA `test_minimax_h3` on dgx (Marlin via `marlin_gemms`), capture delta + s/step; fp4-resident real-checkpoint t2va e2e. Detail: benchmark-record + spec §8 |
