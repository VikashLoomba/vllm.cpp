...
(#170) are `READY` on an accepted
[spike spec](../.agents/specs/container-images.md), none built. See
...
| Qwen3.6-35B-A3B (NVFP4, GDN MoE) | Correctness-complete; binding grid @`a0fa12c7` FLAT 0.935x-0.979x; per-token MoE; gate_up dense +1.31% and down bf16 +2.05% LANDED (bit-exact); PSS 3.81x | Token-exact SYNC+ASYNC; `VT_ASYNC_DEVICE_MIRROR` ON fixes async batch-1 token-0 degeneration; `VT_ASYNC_EXECUTOR` Option A NEUTRAL → OFF |
