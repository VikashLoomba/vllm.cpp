...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **t2va renders a COHERENT scene on GB10 (render bug CLOSED)** | #70/#74 white was WRONG-PARTITION usage (t2va on the ref2va ckpt). t2va on the FL2VA GGUF renders a prompt-matched frame (adj-cos 0.95). spec §8.6. |
...
**DeepSeek-V4-Flash decode levers (2026-08-03): superseded by the 1.14x binding below**, see the record.

**BINDING 2026-08-05: `VT_V4_RESIDENT_W` (default-ON) BEATS ds4, 18.69 vs 16.33 tok/s (1.144x), byte-exact:** staging the dense Q8_0 MLA/shared/lm_head tower device-resident (it was read from GGUF-mmap over ATS) lifts decode 16.23 to 18.69, median-of-3, PEAK flat 86.68 GiB. Mirrors Laguna `VT_LAGUNA_RESIDENT_BF16W`; our GEMV was ATS-bound. **Phase-2 routed-expert residency (`VT_V4_RESIDENT_EXPERTS`) MEASURED NEGATIVE, default-OFF:** byte-exact but -3.4% steady, because the grouped-MoE kernels are dequant/latency-bound, so a bandwidth lever cannot help. Superseded readings and the full forensics are in `.agents/benchmark-record.md`.
...
(2026-08-01 framing superseded by the 2026-08-04 binding line)
...
_(Laguna W8-W11: 0.66 to 0.13 s/tok; W10/W11 superseded by the 2026-08-04 binding result.)_
