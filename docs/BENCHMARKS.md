...
| MiniMax-H3 image conditioning (`row/H3-CONDITIONED-E2E`) | **Vision tower wired to REAL weights.** Reconciled: tower math was CPU-gated SYNTHETIC, never real-wired; encoder GGUF carries the full `visual.*` tower. New GGUF→`Qwen3VLVisionWeights` loader + Qwen3VLProcessor reuse | fl2va/ref2va frame-sanity + the DeepStack scatter into the device text tower recorded in the benchmark record + spec §8.8 |
