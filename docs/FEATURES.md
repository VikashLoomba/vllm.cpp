...
| Parakeet/FastConformer ASR (encoder + CTC, RNN-T and TDT heads) | ctc-0.6b, ctc-1.1b, rnnt-0.6b, tdt-0.6b-v3 transcribed; `.nemo`-only checkpoints need upstream's converter | 18/54859 CPU: stages rel-L2 <= 3.5e-7 vs HF `ParakeetForCTC`/`ForRNNT`/`ForTDT` oracles, CTC and transducer token ids exact, 20/20 mutants killed | none measured, claimed or owed |
