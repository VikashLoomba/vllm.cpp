...
| Parakeet/FastConformer encoder kernels (P1-P3) | **NO number measured, claimed or owed.** Correctness-gated only (byte-identity vs independent scalar refs); no CUDA provider, no GPU suite run. | Floor is `parakeet.cpp`, same clip and box; needs the P4 encoder first |
