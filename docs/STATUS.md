...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **t2va+fl2va COHERENT; ref2va grid = ckpt quant not loader; bf16 shards STREAM: DiT + encoder** | Q4_K_M encoder shifts cond cos 0.9975, DIFFUSE §8.15; ref2va = NVFP4 ckpt fidelity §8.12 **★ ARCH AUDIT:** the C ABI is text-only, so 4 capabilities are embedder-unreachable (`.agents/specs/one-surface-abi.md`). |
