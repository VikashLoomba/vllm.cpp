...
// upstream's own non-contiguous case is the `use_audio_in_video` interleave
// (`common/processor.py:795-812`), which is video and left the row with it.
// `MultiModalFeatureSpec` carries no mask either
// (`include/vllm/multimodal/inputs.h:118-127`), and `try_schedule_encoder_inputs`
// records at `scheduler.cpp:508-512` that the two counts coincide until an
...
// (`common/processor.py:741-748`) and `audio_replacement` (`:768-776`) with the
...
// EARLIER in `updates`, which is upstream's `_next_priority` tie-break
// (`:875-879`) and its documented "the modality that appears earlier takes
// priority" (`:950-953`).
//
...
// same conclusion through `_all_items_found` (`:930-941`). Dropping an item
