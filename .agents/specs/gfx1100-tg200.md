...
- `tests/vt/test_rocm_quant_dot.cpp` historically ran 841 assertions across
  ...
  CPU-only scratch-policy case with 17 assertions. Its four device cases,
  including provider capture and queue-reuse coverage, remain pending the
  operator's HIP gate.
...
eleven formats. Its HIP compile and runtime gate remain pending with the
operator. IQ4_XS remains separate work in #3029 and is not admitted by this
reconstruction. The unmerged campaign's later opt-in arms, default changes,
and product changes are not reachable from this tree. The measured position
and next hypothesis that follow are historical evidence from the source
commit. They are not a current-main benchmark.
