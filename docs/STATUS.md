...
| CPU backend vs llama.cpp | 20-core at floor; RPi5/A76 below floor `GATING` | Pi: AAPCS64 beats SDOT 3.66-5.08%; llama.cpp 2.17x pf / 1.53x dec faster (0.461x/0.653x); RSS -24.2%; 64-tok byte-exact. BF16 GEMM open |
...
| OpenAI server | Supported (subset); #129: SPIKE∅ | `/v1/completions`, `/v1/chat/completions`, streaming SSE, `/v1/models`, `/health`, `/version`, `/ping`, `/metrics`, `/tokenize` (raw-`prompt` and chat-`messages`), `/detokenize`, `/tokenizer_info`, `/server_info`, `/reset_prefix_cache`, `/abort_requests`; `/v1/videos` in OpenAI's Sora shape + `GET /v1/videos/{id}/content`, conditioned on an `input_reference` image or the `metadata` video/audio references. `/tokenizer_info` and `/abort_requests` are flag-gated; `/metrics` and `/reset_prefix_cache` have handlers but no live backing on the async path. Endpoint list and flags: docs/USAGE.md. Depth-2 async serving no longer corrupts the host heap under `ignore_eos` |
| Pooling task class (embeddings / classify / score / rerank) | **PARTIAL; embeddings live on the ONE surface (ROW 6, `57ed063e`)**: `LlamaModel`, PoolingRunner, `vllm_embed` ABI v15 and `/v1/embeddings` | One of eight listed memberships is registered and fixture-gated (engine == direct registry; f64 LAST+normalize). Pending: seven memberships, a real-checkpoint `vllm.LLM(task="embed")` cosine gate, score/rerank/classify endpoints, matryoshka/base64/token-array inputs and tokwise pooling. Detail: `.agents/specs/embeddings-one-surface.md` |
