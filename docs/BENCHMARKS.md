...
| MiniMax-H3 image conditioning (`row/H3-CONDITIONED-E2E`, `row/H3-VISION-SCATTER`) | **Vision→cond scatter WIRED+gated; fl2va COHERENT via encoder path.** merged→prompt_embeds + DeepStack→device text tower (device==host 3.8e-4); fl2va+`--cond-image` matched cat → prompt scene; ref2va STILL grids | grid RE-ATTRIBUTED to the ref2va reference-row assembly, NOT the conditioning (DiT math byte-exact; vision sound per the coherent fl2va A/B); indices {8,16,24} confirmed. §8.9 |
