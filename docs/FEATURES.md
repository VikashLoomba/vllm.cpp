...
| `LlamaModel` | **PARTIAL** (`57ed063e`): synthetic fixture (engine == direct pooler, identical vectors; f64 LAST+normalize reference); real checkpoint (e5-mistral class) and 7 sibling memberships remain | pooling/embed only, text paths refuse by task; `vllm_embed` + `/v1/embeddings` | n/a (CPU correctness-grade embeddings) |
