...
| Qwen3.6-35B-A3B (NVFP4, GDN MoE) | Correctness-complete on SYNC; 3-rep grid 2026-08-05: 0.93-1.03x. ★ async batch-1 greedy DEGENERATES (pre-existing prod bug, repro `1ea26427`); the mirror fixes it | Token-exact greedy SYNC (gate=0); c4 1.025x, c16 0.932x. c16 levers (drain event; `VT_ASYNC_DEVICE_MIRROR`) A/B'd NEUTRAL 2026-08-06, gated OFF; fix = drain-removal + double-buffer; async token gate owed |
