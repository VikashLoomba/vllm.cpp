...
CTest commands recorded in `.agents/state.md`. The 2026-08-06 live-state audit, and wiring it into preflight and CI as a standing gate (`scripts/audit-live-rows.py --check`), are likewise NOT APPLICABLE: moving 10 stale `ACTIVE` rows to `READY` is lifecycle bookkeeping, and the gate is a record checker over matrices and Git refs. No engine code, no kernel, no number on this page: no benchmark movement, nothing to reproduce.
