...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **fp4-resident e2e RUNS on GB10; Marlin W4A16 byte-exact; render coherence OPEN (root-caused to the DiT)** | GB10 leg: Marlin W4A16 byte-exact vs bf16; fp4 a memory win. Real-ckpt fp4 e2e runs (mp4/wav), frame a patch-grid. #70: VAE fine, DiT latent white; ladder gate (`row/H3-DIT-SCALE-GATE`). Detail: spec §8. |
