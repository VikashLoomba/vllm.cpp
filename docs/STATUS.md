...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **Layout + scheduler + DiT forward correctness-complete; end-to-end HARDWARE-BLOCKED** | The project's first DIFFUSION architecture (33.1B CFG-distilled joint video+audio DiT, 50-step flow-matching loop; no KV cache, no sampler, no logits, so it is not a causal-LM registry model). Packed fl2va + ref2va layouts (fp64 position grid BIT-EXACT), latent packing, euler-ancestral eta0 scheduler and the full DiT forward are parity-gated against the UPSTREAM vLLM-Omni modules executed at reduced dimensions: **max abs diff 1.6e-7**, 10/10 cases / 2539 assertions. Reuses the shared `vt::DFlashBlockAttention(causal=false)` for packed varlen non-causal attention — no new kernel. **No e2e or speed claim:** the checkpoint is ~354 GB and upstream validates on 4x B300 at ~133 GB/rank vs one GB10 at 119 GiB unified. Encoder, the two VAEs (checkpoint remote code — must be reimplemented), pipeline, `/v1/videos` + MP4 muxing and the device-resident forward are pending ([spec](../.agents/specs/minimax-h3.md)) |
