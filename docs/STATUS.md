...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **t2va COHERENT on GB10; guard landed; vision tower → real weights** | t2va on FL2VA GGUF renders a matched frame; mismatch RAISES via `_resolve_task`; vision tower now loads from the encoder GGUF. §8.6-8.8. |
