...
CTest commands recorded in `.agents/state.md`. The 2026-08-06 live-state audit, its standing preflight+CI gate (`scripts/audit-live-rows.py --check`), that gate's review follow-up and the accepted session-onboarding design (`.agents/specs/session-onboarding.md`) are likewise NOT APPLICABLE: bookkeeping, a record checker over matrices and Git refs, a role interview, probe 3/5, prose. No engine code, no kernel, no number on this page.
