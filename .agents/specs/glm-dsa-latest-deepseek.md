...
- **O13 — W2 lands three surfaces that NOTHING reaches yet**, and it says so
...
- **O14 — the one staged GGUF arm cannot be fed, because it states no indexer
...
no token, and O13 names the three surfaces nothing reaches yet.
...
then W3. W2's own residue is O13 (the unreached forward) and O14 (the staged
artifact states no indexer schedule, so the arm that exists still cannot be
fed).
