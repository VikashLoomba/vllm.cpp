...
CTest commands recorded in `.agents/state.md`. The 2026-08-06 live-state audit is likewise NOT APPLICABLE: moving all 10 stale `ACTIVE` rows to `READY` across five matrices is lifecycle bookkeeping. It touched no engine code and moved no number on this page: no benchmark movement, nothing to reproduce.
